...
```
pnpm --filter @webpilot/shared test -- --run types-persisted
```
...
```
pnpm --filter @webpilot/extension test -- --run db-v2-migration
```
...
```
pnpm --filter @webpilot/extension test -- --run storage
```
...
```
pnpm --filter @webpilot/extension test -- --run sessions-storage
```
...
```
pnpm --filter @webpilot/extension test -- --run session-store
```
...
export function rehydrateFromPersisted(
  tabId: number,
  data: import("@webpilot/shared/types").PersistedSessionData
): void {
  useStore.setState((state) => {
    const cur = state.sessionsByTab[tabId];
    cur?.abortController?.abort();
...
```
pnpm --filter @webpilot/extension typecheck && \
pnpm --filter @webpilot/extension test
```
...
```
pnpm --filter @webpilot/extension test -- --run auto-persist
```
...
```
pnpm --filter @webpilot/extension test -- --run hydrate
```
...
```
pnpm --filter @webpilot/extension test -- --run tab-close-archiver
```
...
```
pnpm --filter @webpilot/extension test -- --run url-recovery-banner
```
...
```
pnpm --filter @webpilot/extension test -- --run session-history-drawer
```
