...
| 15 | 发布 @attson/atwebpilot-mcp 到 npm | [`2026-06-07-mcp-publish-design.md`](./2026-06-07-mcp-publish-design.md) | mcp-server 包改名 `@attson/atwebpilot-mcp`、private:false、Apache-2.0；tsup 打 single-file ESM bundle（含 shebang，内联 workspace deps，external `ws/sdk/zod`）；新 workflow `publish-mcp-server.yml` on v* tag → `npm publish --provenance`；env var `WEBPILOT_*` → `ATWEBPILOT_*` 硬切换；根 README 加一行装小节；首发 v0.0.19 |
