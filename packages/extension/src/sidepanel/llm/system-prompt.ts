...
const EXAMPLES_ZH = [
  ...
  "示例 4（多 tab）",
  "User: 比较京东和淘宝同款的价格",
  "Plan: extractText 当前 tab 价格 → openTab(淘宝链接) → 等加载 → extractText 价格 → 对比",
  "",
  "示例 5（危险操作 + askUser）",
  "User: 下单这件衣服",
...
export function buildSystemPrompt(input: {
  ...
  if (lang === "en") {
    return [
      ...
      "",
      "=== Tool usage tips ===",
      "- First action on any page: `takeSnapshot` (preferred — UIDs stable, robust to CSS changes) or `snapshotDOM`.",
  ...
  return [
    ...
    "",
    "=== 工具使用建议 ===",
    "- 每次任务起手：takeSnapshot（首选，UID 稳定不怕 class 改名）或 snapshotDOM",
